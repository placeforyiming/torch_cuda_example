    ...
    const int args_1, int args_2)
{
    ...
    torch::Tensor output_3 = torch::full({BATCH_SIZE, args_2}, 0.0, float_opts);
   ...
   const int args_1)
{
    ...
    const int BATCH_SIZE = input_1.size(0);
	const int NUM_CHANNELS = args_1;

	torch::Tensor output_grad_1 = torch::zeros({BATCH_SIZE, 3}, input_1.options());
	torch::Tensor output_grad_3 = torch::zeros({BATCH_SIZE}, input_1.options());
	torch::Tensor output_grad_4 = torch::zeros({BATCH_SIZE, NUM_CHANNELS}, input_1.options());

	return std::make_tuple(output_grad_1, output_grad_3, output_grad_4);
}
